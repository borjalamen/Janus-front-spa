...
export class PeticionComponent implements OnInit {
  ...
  private submitInternal(): void {
    ...
    this.http
      .post(`${environment.baseUrl}peticiones-tareas`, formData)
      .subscribe({
        next: () => {
          this.showToast(
...
@Component({
  ...
  template: `
    <div class="peticion-dialog-container">
      ...
      <p class="dialog-text">
        Estás a un clic de enviar la petición. Confirma que hay permisos de
        imputación en el JIRA propuesto, de lo contrario la petición será
        rechazada.
      </p>
